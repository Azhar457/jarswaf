...
{#if show && preset}
  <div class="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm" in:fade={{ duration: 200 }} out:fade={{ duration: 150 }}>
    <div 
    ...
    >
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
        ...
        <button on:click={() => dispatch('close')} class="text-slate-500 hover:text-slate-300 transition-colors p-1 rounded-md hover:bg-slate-800">
          <X size={20} />
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
        <div class="bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm px-4 py-3 rounded-lg flex items-start gap-3">
          <ShieldAlert size={18} class="shrink-0 mt-0.5" />
          <p>
            You can selectively disable individual signatures within this protection module. Disabling a signature will automatically convert the master wildcard (<span class="font-mono text-xs">{preset.rule_pattern}</span>) into individual rules in your active configuration.
          </p>
        </div>

        <div class="border border-slate-800 rounded-lg overflow-hidden bg-slate-950/30">
          <table class="w-full text-left text-sm">
            <thead class="bg-slate-900/80 border-b border-slate-800 text-slate-400 font-semibold text-xs uppercase tracking-wider">
              <tr>
                  ...
                  <td class="px-4 py-3 text-right">
                    <label class="relative inline-flex items-center cursor-pointer">
                      <input 
                        ...
                        on:change={(e) => dispatch('toggleRule', { ruleId: rule.id, enabled: e.currentTarget.checked })}
                        class="sr-only peer" 
                      />
      ...
      <div class="px-6 py-4 border-t border-slate-800 bg-slate-950/50 flex justify-end">
        <button on:click={() => dispatch('close')} class="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium rounded-lg transition-colors">
          Done
        </button>
      </div>
    </div>
