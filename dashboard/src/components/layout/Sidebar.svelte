...
<aside class={`bg-slate-900 border-r border-slate-800 flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} h-full`}>
  <!-- Logo Area -->
  <div class="h-16 flex items-center justify-between px-4 border-b border-slate-800">
    ...
    {/if}
    <button class="text-slate-400 hover:text-slate-100 p-1 rounded-md hover:bg-slate-800" on:click={() => dispatch('toggleCollapse')}>
      <Menu size={20} />
    </button>
  </div>
