<script lang="ts">
  ...
  $: formattedLogs = $logs.slice(0, 50).map(log => ({
    level: log.action.toLowerCase() === 'block' ? 'ERROR' : 
           log.action.toLowerCase() === 'ratelimit' ? 'WARN' : 'INFO',
...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    <StatCard title="Total Requests" value={$stats.total_requests.toLocaleString()} iconColor="text-blue-500">
      <Activity slot="icon" size={24} />
    </StatCard>
    <StatCard title="Threats Blocked" value={$stats.blocked.toLocaleString()} iconColor="text-red-500">
      <ShieldAlert slot="icon" size={24} />
    </StatCard>
    <StatCard title="Rate Limited" value={$stats.rate_limited.toLocaleString()} iconColor="text-amber-500">
          ...
          {#each $agents as node}
            <tr class="hover:bg-slate-700/30 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <Badge variant={
                  node.status.toLowerCase() === 'online' ? 'success' : 
                  node.status.toLowerCase() === 'warning' ? 'warning' : 'danger'
                }>
                  {node.status}
                </Badge>
              </td>
