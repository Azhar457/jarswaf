...
### 0. Custom Rule Engine — YAML + DSL + Transforms (NEW!)
- **YAML Rule Parser** (`src/rule_engine/`): `serde_yaml`-based rule definition (10 rules: SQLi, XSS, LFI, RCE, Scanner)
- **DSL Parser** (`src/rule_engine/dsl.rs`): `.jwaf` file format — `@id`, `match any/all { field ~ "regex" }`, `---` separator (5 example rules)
...
- **Penetration Tests**: 124 total tests — 25 rule engine + 24 pentest + 6 DSL + 6 transforms + 9 encoded bypass + cache + 54 existing
- **CLI `--rules-dir`**: Load custom rules directory (default `rules/`)
