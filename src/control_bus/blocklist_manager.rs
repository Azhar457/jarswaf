use crate::control_bus::state::{BlockSource, BlocklistEntry, BlocklistSnapshot, IpBlockCount, PublishedState};
...
impl BlocklistManager {
    ...
    pub async fn block_ip(
        ...
        info!("IP {} blocked: {} (source: {:?}, duration: {:?})", ip, reason, source, duration);
    }
    ...
    pub async fn sync_blocklist(&self, ips: Vec<IpAddr>, source: BlockSource) {
        ...
        for ip in &added {
            entries.insert(*ip, BlocklistEntry {
                ip: *ip,
                added_at: now,
