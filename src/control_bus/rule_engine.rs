...
impl RuleEngine {
    ...
    pub async fn add_custom_rule(
        &self,
        rule: CustomRuleDef,
    ) -> Result<String, super::commands::CommandError> {
    ...
    pub async fn update_custom_rule(
        ...
        id: &str,
        rule: CustomRuleDef,
    ) -> Result<(), super::commands::CommandError> {
        let mut rules = self.state.get_rules().as_ref().clone();
        ...
        let existing = rules
            ...
            .find(|r| r.id == id)
            .ok_or_else(|| {
                super::commands::CommandError::NotFound(format!("Rule '{}' not found", id))
            })?;
    ...
    pub async fn add_rate_limit_policy(
        &self,
    ...
    pub async fn remove_rate_limit_policy(
        &self,
        name: &str,
    ) -> Result<(), super::commands::CommandError> {
        let mut rules = self.state.get_rules().as_ref().clone();
    ...
    pub async fn update_vhost(
        ...
        let existing = rules
            ...
            .find(|v| v.name == name)
            .ok_or_else(|| {
                super::commands::CommandError::NotFound(format!("Vhost '{}' not found", name))
            })?;
    ...
    pub async fn top_triggered_rules(
        &self,
        limit: usize,
    ) -> Vec<crate::control_bus::state::RuleTriggerCount> {
    ...
    fn validate_custom_rule(
        ...
        if rule.id.is_empty() {
            return Err(super::commands::CommandError::Validation(
