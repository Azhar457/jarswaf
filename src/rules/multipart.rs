...
pub fn inspect_multipart(body: &[u8], boundary: &str) -> Vec<MultipartFinding> {
    ...
    while i < body.len() {
        ...
        if let Some(pos) = find_subslice(&body[i..], &boundary_bytes) {
            ...
            while header_start < body.len()
                && (body[header_start] == b'\r' || body[header_start] == b'\n')
            {
                header_start += 1;
            }
        ...
        if let Some(ref ctype) = content_type {
            if check_mime_mismatch(part_body, ctype) {
                findings.push(MultipartFinding {
                    ...
                    description: format!(
                        "MIME type mismatch or polyglot file for content-type: {}",
                        ctype
                    ),
                    filename: fname.clone(),
                });
...
--boundary123--\r\n";
        ...
        assert!(findings
            .iter()
            .any(|f| f.rule_id == "MULTIPART-EXEC-CONTENT"));
    }
}
